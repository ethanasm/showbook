...
- **`media.delete` orphaned-blob sweep.** Since 2026-07 the mutation settles all per-variant R2 deletes and drops the DB row regardless (fixing the stranded-row bug), which means a blob whose delete rejected is orphaned with **no DB row referencing it** — unlike `completeUpload` failures, the prune/orphan-media sweep can't find it. Rare (needs an R2 outage exactly at delete time; each failure logs `media.delete.variant_failed` with the key), but a sustained outage silently leaks R2 storage. Durable fix: record failed keys in a small `orphaned_blobs` table (or re-list the R2 prefix in the prune job) so the sweep can retry them.
...
2. ~~Playbill photos - what is this for?~~ *(Answered: theatre-only OCR cast extraction. Image goes to Groq's `qwen/qwen3.6-27b` vision model, comes back as `{actor, role}` pairs, and auto-populates `show_performers` rows with `role: 'cast'` + `characterName`. Image itself is not stored. Verified by `apps/web/tests/playbill-cast-extract.spec.ts`. The schema's existing unused `shows.photos text[]` column is ready for general photos when that lands.)*
