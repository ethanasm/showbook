...
**Playbill cast extraction:** `meta-llama/llama-4-scout-17b-16e-instruct` (Llama 4 Scout) reads photos, extracts cast. Native multimodal — better quality than the older preview models. User confirms before saving. Scout is **deprecated** on Groq (decommission **2026-07-17**) but still serving until then; the durable replacement is the still-supported multimodal `qwen/qwen3.6-27b` (needs an extraction-quality pass first). Do **not** swap to Llama 4 Maverick — Groq already removed it (deprecated 2026-02), so vision calls 404 and the festival/playbill extractors silently return an empty result.
