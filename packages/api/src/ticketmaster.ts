...
// ---------------------------------------------------------------------------
// On-sale status (pure, no network)
// ---------------------------------------------------------------------------
//
// These derive a normalized on-sale status + public-sale date from a raw
// TM event. They live here (not in @showbook/jobs) because they operate
// purely on the `TMEvent` shape owned by this package, and both the
// discover-ingest pipeline and the live performer-upcoming lookup need them.
// `@showbook/jobs` re-exports them for backward compatibility.
