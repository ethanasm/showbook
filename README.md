...
- **Operator-wide LLM spend ceiling** — `SHOWBOOK_LLM_DAILY_BUDGET_USD` (default 5): every Groq call is priced at Groq's per-Mtok rates and metered against this daily all-users budget; once crossed, LLM calls (user-facing and background jobs) are shed until midnight UTC. `SHOWBOOK_ITUNES_LOOKUPS_PER_MINUTE` (default 20) is the deployment-wide iTunes guard matching Apple's per-IP limit. These durable counters live in the `mcpbg_counters` table (auto-created; from `@ethanasm/mcp-budget-governor`) and survive restarts — unlike the per-minute rate-limit buckets, which are in-process.
