...
- `media.upload.{request,ok,no_pending_asset,read_body_failed,r2_write_failed}` — the `PUT /api/media/upload` landing route (the mobile app streams blobs here rather than direct-to-R2). `request` (info) is the unconditional entry log; `ok` (info) is a successful landing (`{key, bytes, elapsedMs}`); `no_pending_asset` (warn, added 2026-07-06) is the security guard rejecting a `PUT` whose key doesn't match a `pending`, quota-reserved `media_assets` variant owned by the caller (409) — the write is also capped at the reserved variant's `bytes`, so neither the count nor byte quota can be bypassed by PUTting outside `createUploadIntent`; `read_body_failed` (error) / `r2_write_failed` (error) are the body-read and object-store boundaries. The pure key→variant matcher is `apps/web/lib/media-upload-auth.ts`.
