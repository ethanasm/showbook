...
- `prune.past_announcements.summary` — the past-announcement phase of `prune/nightly` (deletes announcements whose `show_date < CURRENT_DATE`). Runs before the orphan sweep so it sees the freshly pruned set; the `announcement_has_preserver` rule would otherwise keep past announcements alive forever whenever a user follows the headliner.
