...
- `pnpm verify` — build + lint + typecheck + unit tests + **`pnpm audit --prod`**,
  with an end-of-run status summary. The audit step is a real gate, not
  advisory: between the 2026-06-13 and 2026-08-05 security reviews the
  production advisory count drifted from 2 (dev tooling only) to 22 (3 critical
  / 11 high, several in the live request path) without anything failing, because
...
- `geocode.google.failed`, `geocode.google.no_lat_lng`, `geocode.nominatim.http_error`, `geocode.nominatim.failed` — `geocodeVenue` provider boundaries; were silent until 2026-04-30. `geocode.degraded` (warn) and `geocode.exhausted` (warn) were added 2026-08-06 when `geocodeVenue` moved onto [`provider-router`](https://www.npmjs.com/package/provider-router): `degraded` fires when the answer came from Nominatim (coordinates but no `googlePlaceId` and no `photoUrl`, so venue dedup and the photo backfill are thinner), and `exhausted` replaces the old silent `return null`. The router also emits `router.*` events into this logger at debug. Nominatim's one-call-per-second usage policy is now per-provider pacing (`minInterval: 1.1`) rather than a module-level timestamp every caller shared, and a 429 carrying `Retry-After` opens its circuit for exactly that long on the first refusal.
...
- `venue.placeid.{updated,conflict,failed,missing}` — Google Place-ID resolution inside the `backfill/venue-photos` cron (05:45 ET), fired per venue before the photo fetch. `missing`/`updated` info, `conflict` warn (another venue row already owns the place id), `failed` error.
- `performer.photo.{persisted,persist_failed,persist_image_only,lazy_resolve_failed,refresh_attempt,refresh_failed}` + `performer.photo.proxy.{upstream_error,host_not_allowed,redirect_not_allowed}` — `/api/performer-photo/[performerId]` lazy-resolving SSRF-guarded image proxy (mirrors the venue-photo proxy). Resolves via Wikidata P18 then TM on miss and persists on success; `persist_image_only` (warn) is the TM-id unique-index conflict fallback that writes only the image. The `proxy.*` events match the `venue.photo.proxy.*` semantics.
- `show.cover.{updated,no_match,failed,fatal,done,event_search_failed}` + `backfill.show_cover_images.summary` — daily 06:15 ET `backfill/show-cover-images` cron filling `shows.cover_image_url` for theatre/festival shows via TM event/attraction search (`summary` carries `{total, updated, missing, failed}`). The lazy `/api/show-cover/[showId]` proxy route adds `show.cover.{persisted,persist_failed,lazy_resolve_failed,refresh_attempt,refresh_failed}` and `show.cover.proxy.{upstream_error,host_not_allowed,redirect_not_allowed}`, mirroring the venue-photo / performer-photo proxies.
...
- `backfill.performer_images.summary`, `backfill.performer_mbids.summary`, `backfill.performer_ticketmaster_ids.summary`, `backfill.performer_spotify_ids.summary`, `backfill.venue_photos.summary` — scheduled backfill jobs. `performer-images` (05:30 ET), `venue-photos` (05:45 ET), and `performer-mbids` (04:30 ET) are standalone daily crons; the **TM-id, Spotify-id, and Wikidata-QID** summaries are now emitted by the combined `backfill/performer-ids` sweep (see below), and *also* on operator-triggered runs of the individual queues from the `/admin` page.
- `backfill.performer_ids.summary` — rollup for the combined `backfill/performer-ids` cron (daily 06:00 ET), which resolves the three external catalog IDs in one sweep, in dependency order: Ticketmaster → Spotify → Wikidata, landing before the 07:00 health check (the Wikidata leg used to run at 07:15, *after* the health check, so its result was always a day stale in the morning email). Carries `{tmUpdated, spotifyUpdated, wikidataUpdated, failed}`. Each leg also logs its own `backfill.performer_{ticketmaster_ids,spotify_ids,wikidata_qids}.summary` so per-resolver dashboards keep working; a leg that throws logs `performer.{ticketmaster_id,spotify_id,wikidata_qid}.fatal` and the sweep continues. The individual `backfill/performer-{ticketmaster,spotify,wikidata}-ids` queues stay registered (unscheduled) for `/admin` on-demand triggers.
...
- `media.upload.{request,ok,no_pending_asset,too_large,read_body_failed,r2_write_failed}` — the `PUT /api/media/upload` landing route (the mobile app streams blobs here rather than direct-to-R2). `request` (info) is the unconditional entry log; `ok` (info) is a successful landing (`{key, bytes, elapsedMs}`); `no_pending_asset` (warn, added 2026-07-06) is the security guard rejecting a `PUT` whose key doesn't match a `pending`, quota-reserved `media_assets` variant owned by the caller (409) — the write is also capped at the reserved variant's `bytes`, so neither the count nor byte quota can be bypassed by PUTting outside `createUploadIntent`; `too_large` (warn, added 2026-08-05) is the 413 from that byte ceiling; `read_body_failed` (error) / `r2_write_failed` (error) are the body-read and object-store boundaries. The pure key→variant matcher is `apps/web/lib/media-upload-auth.ts`. The body is read via `apps/web/lib/drain-capped.ts`, which **streams with a cap** instead of `request.arrayBuffer()` — `Content-Length` is advisory (absent on chunked uploads, and forgeable), and App Router handlers impose no body limit, so buffering first let an authenticated caller push an arbitrarily large body into memory before the 413 could fire. It still drains to end-of-stream after overflowing, because answering mid-upload makes iOS NSURLSession report `NSURLErrorCannotParseResponse (-1017)` and mask the real error.
- `media.complete.{ready,failed,head_failed,cleanup_failed}` — `media.completeUpload` outcomes (added 2026-07-02; only `cleanup_failed` existed before, leaving the pending→ready transition invisible). `ready` (info) is the success transition, carries `{assetId, userId, variantCount, bytes}`; `failed` (warn) fires when the size guard marks the asset `failed`, with `reason` (`no_bytes` | `oversize`) plus `{bytes, reservedBytes}`; `head_failed` (error) is an R2 HEAD failure during verification, logged with `{err, assetId, variant}` then rethrown (asset stays `pending`; the prune/orphan-media sweep collects it after 24h); `cleanup_failed` (warn) is a per-variant R2 delete rejection during oversize cleanup.
- `media.delete.{done,variant_failed}` — `media.delete` mutation (added 2026-07-02 alongside the fix that made blob deletes `Promise.allSettled` so one R2 rejection can't strand the DB row with half-deleted blobs — same pattern as `prune.orphan_media.delete_failed`). `variant_failed` (warn) fires per rejected variant delete with `{err, assetId, key}`; `done` (info) carries `{assetId, userId, variantsDeleted, variantsFailed}`.
...
- `admin.sql.{query,error,timeout,rate_limited,config_error,write_attempted}` — the bearer-auth'd read-only `POST /api/admin/sql` diagnostic endpoint (`ADMIN_QUERY_TOKEN`). `query` (info) logs `{ipKey, queryLength, rowCount, truncated, elapsedMs}` — the query *length*, never the SQL text; `timeout` (warn) is SQLSTATE 57014 → 504; `rate_limited` (warn) is the per-IP 30/60s bucket; `write_attempted` (warn) is the read-only-transaction rejection (25006/42501 → 422); `config_error` (error) means the token is unset/too short.
...
- `spotify.request.retry` (warn, added 2026-08-11) — `fetchWithTransientRetry` in `packages/api/src/spotify.ts` retrying a transient transport failure (undici `TypeError: fetch failed` / ETIMEDOUT / `AbortSignal.timeout` TimeoutError, detection shared via `transient-fetch.ts`) on a fresh connection; carries `{call, code}` where `call` is `spotifyFetch` | `refreshSpotifyToken` | `exchangeAuthorizationCode` | `getAppAccessToken`. Mirrors `places.request.retry` / `wikidata.request.retry` / `setlistfm.request.retry`. Added after a single connect-phase ETIMEDOUT on the token refresh aborted a user's hype-playlist export (2026-08-07) — the retried call would have succeeded, as the user's manual retry 30s later did. Playlist-mutating POSTs (`createPlaylist`, `addTracksToPlaylist`, `saveTracksToLibrary`, `replacePlaylistItems`) are deliberately not retried: a request that died mid-flight may have been applied server-side.
- `spotify.playlist.{hype_created,hype_reused,heard_created,heard_reused}` — `spotify.createHypePlaylist` / `spotify.createHeardPlaylist` tRPC mutation outcomes (Phase 3), the **single** success/reuse log per call (the helper's duplicate `spotify.{hype,heard}_playlist.{created,reused}` logs in `spotify-playlist.ts` were removed 2026-07-02; their extra fields moved here). `*_created` fires when a brand-new Spotify playlist is written and carries `{playlistId, trackCount, requested, missingCount, cacheHits, cacheMisses, durationMs}`; `*_reused` fires on the idempotency short-circuit (existing `show_spotify_playlists` row) and omits the resolution telemetry. Paired with the per-stage failure events in `spotify-playlist.ts` (`spotify.hype_playlist.failed`, `spotify.heard_playlist.failed` — carry a `stage` field: `resolve` | `create` | `add_tracks`).
- `spotify.track_resolve.{hit,miss,failed}` — per-track title→URI resolution inside `spotify-playlist.ts`. `hit`/`miss` are debug (source: songs table / memory cache / Spotify search); `failed` (error) is a thrown Spotify search error — the track lands in the playlist's `missing` list and the cache is not poisoned.
- `spotify.recently_played.{summary,bucketed,no_token,no_data,fetch_failed,failed}` — `spotify/recently-played` cron (09:00 ET nightly): pulls each connected user's `/me/player/recently-played` and buckets plays into per-show prep/post counts (±6h of show date, frozen 6h after). `summary` carries `{attempted, matched, noMatch, failed}`; `bucketed` (info) is the per-show write; `fetch_failed`/`failed` are error.
- `spotify.purge_revoked.summary` — weekly `spotify/purge-revoked-tokens` cron (Sun 02:00 ET) hard-deleting token rows revoked >30 days ago.
- `spotify.follow_diff.{served,empty,failed}` — `setlistIntel` followed-artists diff read path (`served`/`empty` info, `failed` error).
- `year_end_soundtrack.{summary,built,no_token,no_candidates,top_tracks_failed,existing_404,failed}` — `spotify/year-end-soundtrack` cron (Dec 31 03:00 ET) building each user's "Showbook · <year>" playlist of one signature track per attended show, idempotent via `users.spotify_year_playlists`. `built` covers both fresh and reused (`outcome: 'fresh' | 'reused'`, `{userId, playlistId, year, trackCount}`); `top_tracks_failed`/`existing_404` warn; `failed` error; `summary` rolls up `{attempted, built, reused, skipped, failed, year}`.
...
- `itunes.preview.{rate_limited,user_rate_limited,failed}`, `itunes.request.{error,parse_error,network_error}` — Apple iTunes Search API fallback used when Spotify returns a track without `preview_url`. `rate_limited` is Apple's IP-level 403 (~20/min); `user_rate_limited` (name kept for dashboard continuity, 2026-08-06) is now the **deployment-wide** resolver guard (`SHOWBOOK_ITUNES_LOOKUPS_PER_MINUTE`, default 20/min, matching Apple's per-IP scope — the old per-user bucket let N users collectively blow through it). Both leave `songs.preview_resolved_at` null so the next tap can retry instead of caching the transient miss.
- `llm.budget.{tripped,rejected}`, `llm.spend.meter_failed`, `budget.schema_init_failed` — the durable budget layer (`packages/api/src/budget.ts`, backed by `mcp-budget-governor` over Postgres; counters in `mcpbg_counters`). Every Groq call through `withLlmRetry` is priced (Groq per-Mtok rates) and metered against the all-users daily `SHOWBOOK_LLM_DAILY_BUDGET_USD` (default $5). `tripped` (warn) fires exactly once per day, on the call that crosses the ceiling; `rejected` (warn) fires per call shed by the pre-flight gate — user-facing procedures and pg-boss jobs alike; `meter_failed` (warn) means spend went **unmetered** (unpriced model or backend blip — a spike is the signal to check the price table in budget.ts); `schema_init_failed` (warn) is the boot-time `mcpbg` schema creation failing (the layer then fails open per call). The library's own events (`governor.limit_exceeded`, `governor.breaker_tripped`, `governor.backend_unavailable`) ride along under `component: budget-governor`.
...
- `setlist.predict.{served,cache_hit,cache_miss,failed,snapshot_failed}` / `setlist.predict_festival.{served,corpus_fill_skipped,corpus_fill_enqueue_failed}` — predicted-setlist read path (`setlist-predict.ts` + `routers/setlist-intel.ts`) on show-detail / festival reads. `served`/`cache_*` info; `failed`/`snapshot_failed` error; `corpus_fill_enqueue_failed` warn (the festival read couldn't enqueue the backing corpus fill).
- `setlist.tour_watch.{summary,refreshed,failed}` — `enrichment/setlist-tour-watch` cron (every 3h) refreshing setlist corpora for performers with upcoming watched shows, deduped per performer via `last_watch_refresh_at`. `failed` is error.
- `setlist.album_drop.boosted` — synthetic album-drop scoring (`album-drop-synthetic.ts`) boosting new-album tracks in a prediction.
- `setlist.special_event.{matched,lookup_failed}` — special-event rule matching in prediction (`matched` info; `lookup_failed` error from the rules read in `routers/setlist-intel.ts`). Admin CRUD on the rules table logs `eval.special_event_rule.{created,updated,deleted}` (info, `{ruleId}`).
- `setlist.song_index.{built,partial,failed}` — per-performer song-index (materialized corpus) rebuild in `song-index-rebuild.ts` (`partial` warn, `failed` error).
- `setlist_intel.show_tab.viewed`, `songs.{page,detail}.viewed` — client view telemetry relayed through `/api/telemetry/*` routes (show-tab tab views; songs surface views, `{userId, songId?}`). Info.
- `setlistIntel.fan_loyalty.{computed,failed}`, `setlistIntel.save_discovered.{ok,failed,not_in_user_history}`, `setlistIntel.discovered_live.failed` — Spotify music-layer intel: fan-loyalty computation, saving a discovered-live song to history (`not_in_user_history` warn), and the discovered-live surface read (`failed` error). Note the camelCase `setlistIntel.` prefix (vs the snake `setlist_intel.show_tab.viewed`).
...
- `festival.lineup.extract.llm_failed` / `playbill.cast.extract.llm_failed` (warn, `component: 'groq.vision'`) — fired from `groq.ts` when the vision/text Groq call is swallowed into an empty result rather than surfaced as an error. `festival.lineup.extract.llm_failed` is `safeExtractFestivalLineup`'s catch (carries `{source: 'image'|'pdf', model, err}`); `playbill.cast.extract.llm_failed` is `extractCast`'s `LlmValidationError` soft-fail (carries `{model, err}`). They exist because both image callers collapse a dead/failing model into "no artists / no cast found" — the regression behind the Scout→Maverick→Qwen vision migration (2026-06-18) only surfaced via a user report because nothing logged. A run-wide spike of either means the vision model (`MODEL_VISION` in `groq.ts`) is broken, not that posters/playbills suddenly lack content.
...
- `account.delete.no_op` (warn, `{userId}` — a delete request found nothing to delete), `account.export.requested` (info, `/api/account/export` data-export route) — account lifecycle odds and ends beyond `auth.user_deleted`.
