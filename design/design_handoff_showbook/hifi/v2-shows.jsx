...
const { SB, Icon, HIFI_KINDS, HIFI_PAST, HIFI_UPCOMING, HIFI_RHYTHM, HIFI_TOTALS, V2Sidebar } = window;

// Combine past + upcoming into one sorted stream (newest-first for list,
// oldest-first fine for calendar)
const ALL_SHOWS = [...HIFI_PAST.map(s=>({...s, when:'past'})), ...HIFI_UPCOMING.map(s=>({...s, when:'up'}))];

// Extended archive to fill the ledger — plausible rows pulled from the fuller archive
const EXTRA_PAST = [
  { id:'p6', kind:'concert',  date:{y:2026,m:'JAN',d:'18',dow:'SAT'}, headliner:'Jeff Rosenstock', support:[], venue:'Warsaw', neighborhood:'Greenpoint', seat:'GA', paid:45 },
  { id:'p7', kind:'theatre', date:{y:2025,m:'DEC',d:'29',dow:'SUN'}, headliner:'Gypsy', support:[], venue:'Majestic Theatre', neighborhood:'Midtown', seat:'ORCH E · 12', paid:189 },
  { id:'p8', kind:'comedy',   date:{y:2025,m:'DEC',d:'11',dow:'THU'}, headliner:'Natasha Leggero', support:['Moshe Kasher'], venue:'Town Hall', neighborhood:'Midtown', seat:'MEZZ A · 4', paid:62 },
  { id:'p9', kind:'concert',  date:{y:2025,m:'NOV',d:'22',dow:'SAT'}, headliner:'Wednesday', support:['MJ Lenderman'], venue:'Bowery Ballroom', neighborhood:'Lower East Side', seat:'GA', paid:35 },
  { id:'p10',kind:'concert',  date:{y:2025,m:'OCT',d:'30',dow:'THU'}, headliner:'Big Thief', support:[], venue:'Webster Hall', neighborhood:'East Village', seat:'GA BALC', paid:72 },
  { id:'p11',kind:'festival', date:{y:2025,m:'SEP',d:'06',dow:'SAT'}, headliner:'Electric Zoo', support:['day 2'], venue:'Randall\'s Island', neighborhood:'Randall\'s Island', seat:'GA', paid:155 },
  { id:'p12',kind:'concert',  date:{y:2025,m:'AUG',d:'14',dow:'THU'}, headliner:'Black Country, New Road', support:[], venue:'Music Hall of Williamsburg', neighborhood:'Williamsburg', seat:'GA', paid:40 },
];
const LEDGER_ROWS = [...HIFI_PAST, ...EXTRA_PAST];
...
function ShowsHeader({mode, onMode, totalShown}) {
  ...
  const modes = [
    { k:'list',     l:'List',     Ic:Icon.Archive,  count:'91' },
    { k:'calendar', l:'Calendar', Ic:Icon.Calendar, count:'4 up' },
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{display:'flex', alignItems:'center', gap:0, border:`1px solid ${RULE2}`}}>
          {['All','2026','2025','2024','older'].map((y,i)=>{
            const active = y==='2026';
            return (
              <div key={y} style={{
                padding:'5px 11px',
                borderRight: i===4 ? 'none' : `1px solid ${RULE2}`,
                background: active?INK:'transparent',
                color: active?BG:INK,
        ...
        <div style={{display:'flex', alignItems:'center', gap:8}}>
          {Object.entries(HIFI_KINDS).map(([k,v])=>(
            <span key={k} style={{
              ...
              <span style={{width:7, height:7, borderRadius:999, background:window.kindInk(k,true)}}/>
              {v.label.toLowerCase()}
            </span>
          ))}
        </div>
...
function ListMode() {
  const M='dark';
  const SURF=SB.surface[M], INK=SB.ink[M], MUTED=SB.muted[M], FAINT=SB.faint[M];
  const RULE=SB.rule[M], RULE2=SB.ruleStrong[M];
  // insert upcoming row group at top
  return (
    <div style={{flex:1, minHeight:0, overflow:'auto', background:SB.bg[M]}}>
      {/* Upcoming strip */}
      <div style={{padding:'18px 36px 8px', display:'flex', alignItems:'baseline', gap:10}}>
        <div style={{fontFamily:SB.mono, fontSize:11, color:INK, letterSpacing:'.1em', textTransform:'uppercase', fontWeight:500}}>
          Upcoming · 4
        </div>
        <div style={{fontFamily:SB.mono, fontSize:10.5, color:FAINT}}>next 5 weeks</div>
      </div>
      <div style={{
        margin:'4px 36px 22px', background:SURF,
        display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:1, backgroundColor:RULE,
      }}>
        {HIFI_UPCOMING.map(u=>(
          <div key={u.id} style={{
            padding:'14px 16px', background:SURF, borderLeft:`2px solid ${window.kindInk(u.kind,true)}`,
          }}>
            <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between'}}>
              <span style={{fontFamily:SB.mono, fontSize:10, color:window.kindInk(u.kind,true), letterSpacing:'.1em', textTransform:'uppercase', fontWeight:500}}>
                {HIFI_KINDS[u.kind].label}
              </span>
              <span style={{fontFamily:SB.mono, fontSize:10, color:u.hasTix?INK:MUTED, letterSpacing:'.04em', display:'inline-flex', alignItems:'center', gap:4}}>
                {u.hasTix ? <><Icon.SquareFilled size={8} color={INK}/>tix</> : <><Icon.Eye size={10} color={MUTED}/>watch</>}
              </span>
            </div>
            <div style={{fontFamily:SB.sans, fontSize:15, fontWeight:600, color:INK, letterSpacing:-0.3, marginTop:6, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>
              {u.headliner}
            </div>
            <div style={{fontFamily:SB.sans, fontSize:12, color:MUTED, marginTop:2, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>
              {u.venue}
            </div>
            <div style={{marginTop:10, display:'flex', alignItems:'baseline', justifyContent:'space-between'}}>
              <span style={{fontFamily:SB.mono, fontSize:11, color:INK, fontWeight:500}}>{u.date.m} {u.date.d}</span>
              <span style={{fontFamily:SB.mono, fontSize:10, color:FAINT}}>{u.countdown}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Past ledger */}
      <div style={{padding:'6px 36px 8px', display:'flex', alignItems:'baseline', gap:10}}>
        <div style={{fontFamily:SB.mono, fontSize:11, color:INK, letterSpacing:'.1em', textTransform:'uppercase', fontWeight:500}}>
          Past · 87
        </div>
        <div style={{fontFamily:SB.mono, fontSize:10.5, color:FAINT}}>newest first</div>
      </div>
      <div style={{margin:'4px 36px 36px', background:SURF}}>
        <div style={{
          display:'grid', gridTemplateColumns:'80px 110px 1.2fr 1fr 110px 64px 30px', columnGap:16,
        ...
        {LEDGER_ROWS.map(s=>(
          <div key={s.id} style={{
            display:'grid', gridTemplateColumns:'80px 110px 1.2fr 1fr 110px 64px 30px', columnGap:16,
            padding:'13px 20px', borderBottom:`1px solid ${RULE}`, alignItems:'center',
          }}>
            <div>
            ...
            <div style={{
              ...
              fontFamily:SB.mono, fontSize:10.5, color:window.kindInk(s.kind,true),
              letterSpacing:'.08em', textTransform:'uppercase', fontWeight:500,
            }}>
              <span style={{width:6, height:6, borderRadius:999, background:window.kindInk(s.kind,true)}}/>
            ...
            <div style={{textAlign:'right', fontFamily:SB.mono, fontSize:12, color:INK, fontWeight:500, fontFeatureSettings:'"tnum"'}}>
              ${s.paid}
            </div>
            <Icon.ChevronRight size={14} color={FAINT}/>
          </div>
        ))}
        <div style={{
      ...
      {mode==='list' && (
        <div style={{flex:1, overflow:'auto'}}>
          <div style={{padding:'8px 20px 6px', display:'flex', alignItems:'baseline', justifyContent:'space-between'}}>
            <div style={{fontFamily:SB.mono, fontSize:10.5, color:INK, letterSpacing:'.1em', textTransform:'uppercase', fontWeight:500}}>Upcoming · 4</div>
            <div style={{fontFamily:SB.mono, fontSize:10, color:MUTED}}>next 5 wks</div>
          </div>
          {HIFI_UPCOMING.slice(0,3).map(u=>(
            <div key={u.id} style={{
              margin:'6px 20px 6px', padding:'12px 14px', background:SURF,
              borderLeft:`2px solid ${kInk(u.kind)}`,
              display:'grid', gridTemplateColumns:'52px 1fr auto', columnGap:12, alignItems:'start',
            }}>
              <div>
                <div style={{fontFamily:SB.sans, fontSize:20, fontWeight:500, color:INK, letterSpacing:-0.6, lineHeight:.95, fontFeatureSettings:'"tnum"'}}>{u.date.d}</div>
                <div style={{fontFamily:SB.mono, fontSize:9.5, color:kInk(u.kind), letterSpacing:'.06em', marginTop:3, textTransform:'uppercase', fontWeight:500}}>{u.date.m}</div>
              </div>
              <div style={{minWidth:0}}>
                <div style={{fontFamily:SB.sans, fontSize:14, fontWeight:600, color:INK, letterSpacing:-0.2, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{u.headliner}</div>
                <div style={{fontFamily:SB.mono, fontSize:10, color:MUTED, marginTop:3}}>{u.venue.toLowerCase()}</div>
              </div>
              <div style={{fontFamily:SB.mono, fontSize:9.5, color: u.hasTix ? INK : MUTED, display:'inline-flex', alignItems:'center', gap:4, letterSpacing:'.04em'}}>
                {u.hasTix?'tix':'watch'}
              </div>
            </div>
          ))}

          <div style={{padding:'18px 20px 6px', borderTop:`1px solid ${RULE2}`, marginTop:10, display:'flex', alignItems:'baseline', justifyContent:'space-between'}}>
            <div style={{fontFamily:SB.mono, fontSize:10.5, color:INK, letterSpacing:'.1em', textTransform:'uppercase', fontWeight:500}}>Past · 87</div>
            <div style={{fontFamily:SB.mono, fontSize:10, color:MUTED}}>newest first</div>
          </div>
          {LEDGER_ROWS.slice(0,7).map(s=>(
            <div key={s.id} style={{
              padding:'13px 20px', borderTop:`1px solid ${RULE}`,
              display:'grid', gridTemplateColumns:'42px 1fr auto', columnGap:12, alignItems:'start',
            }}>
              <div>
                <div style={{fontFamily:SB.sans, fontSize:20, fontWeight:500, color:INK, letterSpacing:-0.6, lineHeight:.95, fontFeatureSettings:'"tnum"'}}>{s.date.d}</div>
                <div style={{fontFamily:SB.mono, fontSize:9.5, color:FAINT, marginTop:3}}>{s.date.m.toLowerCase()}</div>
              </div>
              <div style={{minWidth:0}}>
                <div style={{display:'inline-flex', alignItems:'center', gap:5,
                  fontFamily:SB.mono, fontSize:9.5, fontWeight:500, color:kInk(s.kind),
                  letterSpacing:'.04em', textTransform:'lowercase'}}>
                  <span style={{width:5, height:5, borderRadius:999, background:kInk(s.kind)}}/>
                  {HIFI_KINDS[s.kind].label}
                </div>
                ...
                <div style={{fontFamily:SB.mono, fontSize:10, color:MUTED, marginTop:3}}>{s.venue.toLowerCase()}</div>
              </div>
              <div style={{fontFamily:SB.mono, fontSize:11, color:INK, fontWeight:500, fontFeatureSettings:'"tnum"'}}>${s.paid}</div>
            </div>
          ))}
