/**
 * Brand-recognizable SVG marks for external-link affordances. Using the
 * official-style Spotify/Ticketmaster marks (rather than generic lucide
 * icons) makes it obvious at a glance that a tap opens the third-party
 * app/site.
 */
...
/**
 * The lucide-react-native icon prop signature, accepted (and ignored
 * apart from `size`) so these marks are drop-in compatible anywhere a
 * lucide icon is plumbed through via a `ComponentType<…>` slot.
 */
interface MarkProps {
  size?: number;
  color?: string;
...
export function SpotifyMark({ size = 14 }: MarkProps): React.JSX.Element {
  return (
...
export function TicketmasterMark({ size = 14 }: MarkProps): React.JSX.Element {
  return (
    <Svg width={size} height={size} viewBox="0 0 32 32">
      <Rect width={32} height={32} rx={6} fill="#026CDF" />
      <SvgText
        x={16}
        y={22}
        textAnchor="middle"
